...
{
    public partial class certificationd : Page
    {
        public string admin = "0";
        public int back;
        public string criteria = "";
        public string criteria_type = "";
        public string data_status = "Deferred";
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (this.selectSearchCriteria.SelectedValue == "product_title")
            {
                ...
                if (this.lt_m.Count> 0)
                {
                    this.Session["x_cnt"] = this.x_cnt = this.lt_m.Count;
                    this.Session["lt_m"] = this.lt_m;
                    this.gvX.DataSource = this.lt_m;
                    this.gvX.DataBind();
                    this.criteria = string.Concat(new object[] { this.lt_m.Count, " result(s) found for search criteria (", this.kword.Value, ")!!" });
                }
                else
                {
        ...
        protected void gvX_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "TmViewClick")
            {
                ...
                base.Response.Redirect("certificationd_data_detailsx.aspx?x=" + str);
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.max = Convert.ToInt32(ConfigurationManager.AppSettings["c_unit_limit"]);
            if (!base.IsPostBack)
            {
